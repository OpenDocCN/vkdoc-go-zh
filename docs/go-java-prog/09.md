...
这种方法提供了更大的灵活性，尤其是在(1)测试代码(可以注入模拟对象)或者(2)配置对象之间的复杂关系时。第二种情况在 Java 开发中非常普遍，以至于一个主要的框架， *Spring* 、<sup>、 3 、T5*Spring Boot*、 <sup>4</sup> 就是为了提供这种情况而创建的。其他选项也存在，比如谷歌的 Guice。 <sup>5</sup></sup>

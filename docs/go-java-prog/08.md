...
Java 允许声明类似常量的 <sup>1</sup> ( `static final`)值。Go 有真常数。Go 支持一次定义一个常量并分组定义。在 Java 和 Go 中，初始值都是必需的。
...
注意在作者看来，绝对不应该使用`goto`语句；`if`、`switch`、`for`提供足够的本地控制流量。您的代码应该遵循结构化编程的原则 <sup>2</sup> 及其 Goto <sup>3</sup> -less 方法。
...
[T2`https://en.wikipedia.org/wiki/Structured_programming`；](https://en.wikipedia.org/wiki/Structured_programming) [` en。维基百科。org/wiki/Structured _ program _ theory`](https://en.wikipedia.org/wiki/Structured_program_theorem)

  3

[` en。维基百科。org/ wiki/ Goto`](https://en.wikipedia.org/wiki/Goto)

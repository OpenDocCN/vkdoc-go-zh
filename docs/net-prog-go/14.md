...
这与第十章中描述的系统略有不同。主要的结构差异是每张卡都有自己的 URL 作为抽认卡组的成员。
...
这些都没有什么特别新的东西。我们还没有完成一些操作的代码，比如`DELETE`:这些返回 HTTP 代码 501 `'Not implemented'`。我们也以`text/plain`的形式返回各个卡片的内容:它们有一个复杂的 JSON/Go 结构，如第十章中所使用的，但这对于讨论这个系统的其他方面是不需要的。

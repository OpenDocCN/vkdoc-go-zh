...
这个系统的最后一部分是如何在浏览器中显示这个 HTML。图 10-3 显示了一个由四部分组成的屏幕，显示英语、简体中文、备选翻译和繁体/简体对。这是如何通过下载到服务器的 JavaScript 程序完成的(这是使用`FileServer` Go 对象完成的)。JavaScript `slideviewer.js`文件实际上很长，因此在文本中被省略了。它包含在 [`http://www.apress.com/9781484226919`](http://www.apress.com/9781484226919) 的程序文件中。

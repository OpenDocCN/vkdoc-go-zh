...
一旦获得了一个`Session`，就可以对一个`Collection`值执行 CRUD 操作。让我们编写一个示例程序来演示针对`Collection`值的持久性和读取操作。首先，我们在两个源文件中编写示例程序:`bookmark_store.go`和`main.go`。清单 [6-1](#Par24) 显示了`bookmark_store.go`文件的源代码，该文件包含一个用于定义数据模型的名为`Bookmark`的结构，以及一个为执行 CRUD 操作提供持久性逻辑的结构类型`BookmarkStore`。
...
名为`BookmarkStore`的结构被声明用于提供持久性逻辑，该逻辑使用结构`Bookmark;`进行插入和更新操作，它接受值`it`，对于读取操作，它返回相同类型的值。结构`BookmarkStore`有一个带`type mgo.Collection`的字段`C`。通过访问字段`C`执行所有 CRUD 操作。从源文件`main.go`(见清单 [6-2)，](#Par49)`Collection`对象被提供给`BookmarkStore`值，并通过访问`BookmarkStore`的方法执行 CRUD 操作。
...
让我们编写一个示例程序来演示使用 RethinkDB 的持久性和读操作。让我们在两个源文件中编写示例程序:`bookmark_store.go`和`main.go`。清单 [6-3](#Par85) 显示了`bookmark_store.go`文件的源代码，该文件包含一个用于定义数据模型的名为`Bookmark`的结构，以及一个为针对名为`"bookmarks"`的表执行 CRUD 操作提供持久性逻辑的结构类型`BookmarkStore`。
...
名为`BookmarkStore`的结构被声明用于提供使用数据模型结构`Bookmark`的持久性逻辑。结构`BookmarkStore`有一个类型为`gorethink.Session`的字段`Session`。所有 CRUD 操作都是通过访问使用字段`Session`的`BookmarkStore`的方法来执行的。
